...
from bitagent.validator.reward import (
    process_rewards_update_scores_for_many_tasks_and_many_miners,
)
    ...
    while True:
        try:
            if timeout and time.time() - start_time > timeout:
                bt.logging.error(
                    f"OFFLINE: Server did not become ready within timeout period"
                )
            ...
            if not procutil.is_running():  # Check if process is still running
                bt.logging.error(
                    f"OFFLINE: Server process terminated unexpectedly, check VRAM usage"
                )
                raise Exception(
                    "Server process terminated unexpectedly, potentially VRAM usage issue"
                )
            if server_process.poll() is not None:
                bt.logging.error(
                    f"OFFLINE: Server process terminated with code {server_process.poll()}"
                )
                raise Exception(
                    f"Server process terminated with code {server_process.poll()}"
                )
...
async def offline_task(self, wandb_data):
    ...
    bt.logging.debug(
        f"OFFLINE: Miners needing model lookup: {miners_needing_model_lookup}"
    )
    bt.logging.debug(f"OFFLINE: sending request to miners: {self.metagraph.axons}")
            ...
            for i, uid in enumerate(miners_needing_model_lookup):
                ...
                if "/" in hf_model_name:
                    try:
                        ...
                        bt.logging.error(
                            f"OFFLINE: Error getting model info for {hf_model_name}: {e}"
                        )
    ...
    for uid in miner_uids:
        existing_model_name = self.offline_model_names[self.competition_version].get(
            uid, ""
        )
    ...
    if not miner_hf_model_names:
        bt.logging.info(
            f"OFFLINE: No unique miner HF model names to evaluate in OFFLINE mode"
        )
    ...
    unique_miner_hf_model_names = [
        m for m in miner_hf_model_names if m not in [None, ""]
    ]

    bt.logging.debug(
        f"OFFLINE: Unique miner HF model names: {len(unique_miner_hf_model_names)}"
    )
    ...
    for hfmn in unique_miner_hf_model_names:
        ...
        for k, model_name in self.offline_model_names[self.competition_version].items():
            if model_name == hfmn:
                ...
                scores_with_same_model.append(
                    self.offline_scores[self.competition_version][k]
                )

        if len(uids_with_same_model) > 0:
            max_score_for_model = max(
                scores_with_same_model
            ...
            bt.logging.debug(
                f"OFFLINE: Found miner with same model, using existing score"
            )
            for uid in the_uids:
    ...
    unique_miner_hf_model_names = [
        m for m in unique_miner_hf_model_names if m not in models_to_skip
    ]
    ...
    if len(unique_miner_hf_model_names) > 0:
        ...
        tasks = [
            ToolCallTask(
                validator=self, name="Responds with correct function call", offline=True
            )
    ...
    for i, hf_model_name in enumerate(unique_miner_hf_model_names):
        bt.logging.debug(
            f"OFFLINE: Running tasks for model {i + 1} of {len(unique_miner_hf_model_names)}"
        )
        ...
        if (
            ...
            or hf_model_name == ""
            or hf_model_name.lower() == "none"
        ):
            bt.logging.debug(
                f"OFFLINE: Miner returned empty HF model name ... skipping"
            )
        ...
        try:
            ...
            bt.logging.debug(f"OFFLINE: got model license: {license}")
        except Exception:
        ...
        if license not in ["apache-2.0", "cc-by-nc-4.0", "mit"]:
            bt.logging.debug(
                f"OFFLINE: Skipping model {i + 1} of {len(unique_miner_hf_model_names)} due to license: {license}"
            )
            for miner_uid in hf_model_name_to_miner_uids[hf_model_name]:
                self.offline_scores[self.competition_version][miner_uid] = 0.0
        ...
        if total_size > 10000000000:
            bt.logging.debug(
                f"OFFLINE: Skipping model {i + 1} of {len(unique_miner_hf_model_names)} due to size: {total_size}"
            )
            for miner_uid in hf_model_name_to_miner_uids[hf_model_name]:
                self.offline_scores[self.competition_version][miner_uid] = 0.0
        ...
        try:
            ...
            try:
                cache_dir = os.path.expanduser(self.config.validator_hf_cache_dir)
                snapshot_dir = (
                    f"{cache_dir}/models--{hf_model_name.replace('/', '--')}/snapshots/"
                )
            ...
            try:
                await asyncio.wait_for(
                    asyncio.to_thread(
                        wait_for_server,
                        f"http://localhost:{self.config.validator_hf_server_port}",
                        server_process,
                    ),
                    timeout=60 * 15,  # wait up to 15 minutes
                )
                bt.logging.debug(
                    f"OFFLINE: Server for model {i + 1} of {len(unique_miner_hf_model_names)} started"
                )
                ...
                bt.logging.error(
                    f"OFFLINE: Timeout waiting for server for model {i + 1} of {len(unique_miner_hf_model_names)} to start, skipping"
                )
            ...
            # likely a validator error
            bt.logging.error(
                f"OFFLINE: Error starting sglang server for model: {i + 1} of {len(unique_miner_hf_model_names)}: {e}"
            )
        ...
        # get LLM responses
        bt.logging.debug(
            f"OFFLINE: Getting LLM responses for model {i + 1} of {len(unique_miner_hf_model_names)}"
        )
        ...
        try:
            ...
            bt.logging.error(
                f"OFFLINE: Error getting LLM responses: {e}, have to skip this model"
            )
            continue
        ...
        bt.logging.debug(
            f"OFFLINE: Got {len(llm_responses)} LLM responses for model: {i + 1} of {len(unique_miner_hf_model_names)}"
        )
        ...
        bt.logging.debug(
            f"OFFLINE: Terminating server for model: {i + 1} of {len(unique_miner_hf_model_names)}"
        )
        wandb_data["event_name"] = "HF Model Eval Server Terminating"
        self.log_event(wandb_data)
        await asyncio.to_thread(terminate_process, server_process)
        bt.logging.debug(
            f"OFFLINE: Terminated server for model: {i + 1} of {len(unique_miner_hf_model_names)}"
        )
        ...
        bt.logging.debug(
            f"OFFLINE: Processing rewards for model: {i + 1} of {len(unique_miner_hf_model_names)}, for miners: {these_miner_uids}"
        )
        ...
        if not latest_snapshot:
            bt.logging.debug(
                f"OFFLINE: Deleting model from HF cache: {i + 1} of {len(unique_miner_hf_model_names)}"
            )
            wandb_data["event_name"] = "Deleting HF Model from Cache"
            self.log_event(wandb_data)
            await asyncio.to_thread(
                delete_model_from_hf_cache, self, hf_model_name.split("@")[0]
            )
        else:
            bt.logging.debug(
                f"OFFLINE: NOT Deleting model from HF cache: {i + 1} of {len(unique_miner_hf_model_names)}"
            )
            wandb_data["event_name"] = (
                "NOT Deleting HF Model from Cache - snapshot found, so no new download to revert"
            )
            self.log_event(wandb_data)
